...
- (void)rightUtilityButtonHandler:(id)sender
{
    ...
    if ([self.delegate respondsToSelector:@selector(swipeableTableViewCell:didTriggerRightUtilityButtonWithIndex:andIndexpath:)])
    {
        [self.delegate swipeableTableViewCell:self didTriggerRightUtilityButtonWithIndex:utilityButtonIndex andIndexpath:self.indexPathCell];
    }
}
...
- (void)hideUtilityButtonsAnimated:(BOOL)animated
{
    if (_cellState != kCellStateCenter)
    {
        ...
        if ([self.delegate respondsToSelector:@selector(swipeableTableViewCell:scrollingToState:andIndexpath:)])
        {
            [self.delegate swipeableTableViewCell:self scrollingToState:kCellStateCenter andIndexpath:self.indexPathCell];
        }
    }
}
...
- (void)showLeftUtilityButtonsAnimated:(BOOL)animated {
    if (_cellState != kCellStateLeft)
    {
        ...
        if ([self.delegate respondsToSelector:@selector(swipeableTableViewCell:scrollingToState:andIndexpath:)])
        {
            [self.delegate swipeableTableViewCell:self scrollingToState:kCellStateLeft andIndexpath:self.indexPathCell];
        }
    }
}
...
- (void)showRightUtilityButtonsAnimated:(BOOL)animated {
    if (_cellState != kCellStateRight)
    {
        ...
        if ([self.delegate respondsToSelector:@selector(swipeableTableViewCell:scrollingToState:andIndexpath:)])
        {
            [self.delegate swipeableTableViewCell:self scrollingToState:kCellStateRight andIndexpath:self.indexPathCell];
        }
    }
}
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
    ...
    if ([self.delegate respondsToSelector:@selector(swipeableTableViewCell:scrollingToState:andIndexpath:)])
    {
        [self.delegate swipeableTableViewCell:self scrollingToState:_cellState andIndexpath:self.indexPathCell];
    }
