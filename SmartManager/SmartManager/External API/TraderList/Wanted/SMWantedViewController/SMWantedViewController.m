...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (tableView == self.tableWanted)
    {
        ...
        if (objectDropDown.isNumber!=nil)
        {
            NSAttributedString *searchAttributedString = [[NSAttributedString alloc]initWithString:[NSString stringWithFormat:@"%d",objectDropDown.isNumber.integerValue] attributes:@{NSUnderlineStyleAttributeName:[NSNumber numberWithInt:NSUnderlineStyleSingle],NSForegroundColorAttributeName:[UIColor whiteColor],NSFontAttributeName:[UIFont fontWithName:FONT_NAME_BOLD size:20.0f]}];
        ...
        else
        {
            ...
            {
                [Fontclass AttributeStringMethodwithFontWithButton:cell.btnSearch iconID:370];
            }
        }
        ...
        {
            cell.separatorInset = UIEdgeInsetsMake(0.f, 0, 0.f, 0.f);
        }
        else
        {
            cell.separatorInset = (UI_USER_INTERFACE_IDIOM()==UIUserInterfaceIdiomPhone) ? UIEdgeInsetsMake(0.f, 0, 335.f, 1.f) : UIEdgeInsetsMake(0.f, 0, 783.f, 1.f);
        }
...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField
{
    ...
    {
        selectedIndex = self.txtFromYear.tag;
    ...
    {
        selectedIndex = self.txtToYear.tag;
    ...
    {
        selectedIndex = self.txtMake.tag;
    ...
    {
        selectedIndex = self.txtModel.tag;
...
- (void)btnDeleteDidClicked:(id)sender
{
    UIButton *btn = (UIButton*)sender;
    
    objectDropDown = [arrayWantedSearch objectAtIndex:btn.tag];
    
    UIBAlertView *alert = [[UIBAlertView alloc] initWithTitle:KLoaderTitle message:@"Are you sure you want to delete this vehicle?" cancelButtonTitle:@"Yes" otherButtonTitles:@"No",nil];
...
qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"SearchResultCountResult"])
    {
        ...
        objectDropDown.isNumber = [NSNumber numberWithInteger:currentNodeContent.integerValue];
...
- (void)swipeableTableViewCell:(SWTableViewCell *)cell didTriggerRightUtilityButtonWithIndex:(NSInteger)index andIndexpath:(NSIndexPath*)indexpath
{
    switch (index)
    {
        case 0:
            ...
            NSLog(@"%d",objectDropDown.iWantedSearchID);
