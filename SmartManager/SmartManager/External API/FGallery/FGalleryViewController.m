...
#define kThumbnailSize 90
#define kThumbnailSpacing 8
...
- (void)loadView
{
    ...
    // setup caption
    _captionContainer.backgroundColor			= [UIColor redColor];
...
- (void)setUseThumbnailView:(BOOL)useThumbnailView
{
    ...
    if( self.navigationController ) {
        if (_useThumbnailView)
        {
            ...
            UIButton *btnSeeAll = [[UIButton alloc] initWithFrame:CGRectMake(0,0,60,35)];
            
            [btnSeeAll addTarget:self action:@selector(handleSeeAllTouch:) forControlEvents:UIControlEventTouchUpInside];
            [btnSeeAll setTitle:NSLocalizedString(@"See all", @"") forState:UIControlStateNormal];
            UIBarButtonItem *barButton = [[UIBarButtonItem alloc] initWithCustomView:btnSeeAll];
            
            UIButton *btnShare = [[UIButton alloc] initWithFrame:CGRectMake(0,0,25,25)];
            [btnShare addTarget:self action:@selector(handleShareTouch:) forControlEvents:UIControlEventTouchUpInside];
            UIBarButtonItem *barButton1 = [[UIBarButtonItem alloc] initWithCustomView:btnShare];
            
            self.navigationItem.rightBarButtonItems=[NSArray arrayWithObjects:barButton,barButton1,nil];

            
            [Fontclass AttributeStringMethodwithFontWithButton:btnShare iconID:458];
            
            
            //[self.navigationItem setRightBarButtonItems:[NSArray arrayWithObjects:btnShare,btnSeeAll, nil]];
...
#pragma mark - Setting navigation title
...
- (void)showThumbnailViewWithAnimation:(BOOL)animation
{
    ...
    [[self.navigationItem.rightBarButtonItems objectAtIndex:0] setTitle:NSLocalizedString(@"Close", @"")];
...
- (void)hideThumbnailViewWithAnimation:(BOOL)animation
{
    ...
    [[self.navigationItem.rightBarButtonItems objectAtIndex:0] setTitle:NSLocalizedString(@"See all", @"")];
...
- (void)handleShareTouch:(id)sender
{
    UIImage *image = imageForSharing;
    if (image!= nil) {
        ...
        if (UI_USER_INTERFACE_IDIOM()==UIUserInterfaceIdiomPad && [[[UIDevice currentDevice]systemVersion]floatValue]>=8.0)
        {
            ...
            [popController presentPopoverFromRect:CGRectMake(234,300,300,300) inView:self.view permittedArrowDirections:0 animated:YES];
        }
        else
        {
...
- (void)galleryPhoto:(FGalleryPhoto*)photo didLoadFullsize:(UIImage*)image
{
	...
	if( _currentIndex == photo.tag )
	{
        ...
        imageForSharing = photoView.imageView.image;
	}
...
- (BOOL)shouldAutorotateToInterfaceOrientation:(UIInterfaceOrientation)interfaceOrientation
{
	...
	return ( interfaceOrientation == UIInterfaceOrientationPortrait ) ? YES : YES;
}
