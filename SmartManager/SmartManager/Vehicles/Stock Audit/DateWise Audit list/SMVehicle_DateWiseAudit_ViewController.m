//
//  SMVehicle_DateWiseAudit_ViewController.m
//  SmartManager
//
...
@interface SMVehicle_DateWiseAudit_ViewController ()

@end

@implementation SMVehicle_DateWiseAudit_ViewController
...
- (void)viewDidLoad
{
    ...
    [self setTheTitle];
    arrayOFDateWiseAudit = [[NSMutableArray alloc]init];
    self.tblViewDateWiseAuditList.tableFooterView = [[UIView alloc]init];

    pageNumberCount=0;
    isLoadMore = NO;

    [self getTheAuditHistoryDetailedList];
    
    self.btnSubmit.layer.cornerRadius = 5.0;
    //self.btnSubmit.titleLabel.font = [UIFont fontWithName:FONT_NAME size:16.0];
...
{
    return [arrayOFDateWiseAudit count];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    SMStockAuditDetailObject *auditTodayObj = [arrayOFDateWiseAudit objectAtIndex:indexPath.row];
    ...
    if (arrayOFDateWiseAudit.count-1 == indexPath.row)
    {
        self.tblViewDateWiseAuditList.tableFooterView = [[UIView alloc]init];
        
        
        if (arrayOFDateWiseAudit.count !=totaCount)
        {
            ++pageNumberCount;
            isLoadMore=YES;
            [self getTheAuditHistoryDetailedList];
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView == self.tblViewDateWiseAuditList)
    {
        if (indexPath.section == 0)
        {
           ...
           // height = ([self heightForText:[NSString stringWithFormat:@"GEO: %@",auditTodayObj.auditGeoAddress]]+[self heightForText:[NSString stringWithFormat:@"%@ | %@ | %@",@"%@", @"ruby", @"smtests3-2"]] +[self heightForText:[NSString stringWithFormat:@"%@ | %@ | %@",auditTodayObj.auditVehicleType, auditTodayObj.auditVehicleMileage, @"165 Days"]]+ (65.0+51.0+7.0));
            ...
            //NSLog(@"auditTodayObj.auditVinID %@",auditTodayObj.auditGeoAddress);
                        return height+30;
        }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [sliderCell.imageVehicle setImage:[UIImage imageNamed:@"carImage.jpg"]];
...
- (IBAction)btnExpandEmailListDidClicked:(id)sender
{
    ...
    [self.tblViewDateWiseAuditList reloadData];
...
- (void)setTheTitle
{
    ...
    listActiveSpecialsNavigTitle.text = self.selectedDateStrForTitle;
...
-(void)getTheAuditHistoryDetailedList
{
    
    [HUD show:YES];
    [HUD setLabelText:KLoaderText];
    
     NSMutableURLRequest *requestURL=[SMWebServices getTheAuditHistoryDetailstWithUserHash:[SMGlobalClass sharedInstance].hashValue andClientId:[SMGlobalClass sharedInstance].strClientID.intValue andDay:self.selectedDateStr andPageNumber:pageNumberCount andRecordCount:10];
    ...
    [NSURLConnection sendAsynchronousRequest:requestURL
                           ...
                           completionHandler:^(NSURLResponse* response, NSData* data, NSError* error)
     {
         ...
         else
         {
             ...
             if (!isLoadMore)
             {
                 [arrayOFDateWiseAudit removeAllObjects];
    ...
    attributes:(NSDictionary *) attributeDict
{
    ...
    {
        self.auditHistoryDetailObj = [[SMStockAuditDetailObject alloc]init];
...
qualifiedName:(NSString *)qName
{
    ...
    {
        self.auditHistoryDetailObj.auditTime = currentNodeContent;
    }
    if([elementName isEqualToString:@"VIN"])
    {
        if([currentNodeContent length] == 0)
        {
            self.auditHistoryDetailObj.auditVinID = @"VIN?";
        }
        else
        {
            self.auditHistoryDetailObj.auditVinID = currentNodeContent;
        }
    }
    if([elementName isEqualToString:@"GEO"])
    {
        self.auditHistoryDetailObj.auditGeoAddress = currentNodeContent;
    }
    if([elementName isEqualToString:@"Make"])
    {
        self.auditHistoryDetailObj.auditVehicleName = currentNodeContent;
    }
    if([elementName isEqualToString:@"Model"])
    {
        self.auditHistoryDetailObj.auditVehicleName = [NSString stringWithFormat:@"%@ %@",self.auditHistoryDetailObj.auditVehicleName,currentNodeContent];
    }
    if([elementName isEqualToString:@"Registration"])
    {
        self.auditHistoryDetailObj.auditVehicleRegNum = currentNodeContent;
    }
    if([elementName isEqualToString:@"RetailPrice"])
    {
        if([currentNodeContent length] == 0 || currentNodeContent.intValue == 0)
            self.auditHistoryDetailObj.auditVehiclePriceRetail = @"R?";
        else
        self.auditHistoryDetailObj.auditVehiclePriceRetail = [[SMCommonClassMethods shareCommonClassManager]priceConvertCurrencyEn_AF:[NSString stringWithFormat:@"%d",currentNodeContent.intValue]];
    }
    if([elementName isEqualToString:@"TradePrice"])
    {
        if([currentNodeContent length] == 0 || currentNodeContent.intValue == 0)
            self.auditHistoryDetailObj.auditVehiclePriceTrade = @"R?";
        else
        self.auditHistoryDetailObj.auditVehiclePriceTrade = [[SMCommonClassMethods shareCommonClassManager]priceConvertCurrencyEn_AF:[NSString stringWithFormat:@"%d",currentNodeContent.intValue]];
    }
    if([elementName isEqualToString:@"Age"])
    {
        self.auditHistoryDetailObj.auditVehicleDays = [NSString stringWithFormat:@"%@ Days",currentNodeContent];
    }
    if([elementName isEqualToString:@"StockNumber"])
    {
        if([currentNodeContent length] == 0)
        {
            self.auditHistoryDetailObj.auditStockNo = @"Stock code?";
        }
        else
        {
            self.auditHistoryDetailObj.auditStockNo = currentNodeContent;
        }
    }
    if([elementName isEqualToString:@"LicenseImage"])
    {
        self.auditHistoryDetailObj.auditLicenseURL = currentNodeContent;
    }
    if([elementName isEqualToString:@"VehicleImage"])
    {
        self.auditHistoryDetailObj.auditVehicleURL = currentNodeContent;
    }
    if([elementName isEqualToString:@"Year"])
    {
        if([currentNodeContent length] == 0)
        {
            self.auditHistoryDetailObj.auditVehicleYear = @"Year?";
        }
        else
        {
            self.auditHistoryDetailObj.auditVehicleYear = currentNodeContent;
        }
    }
    if([elementName isEqualToString:@"Mileage"])
    {
        if([currentNodeContent length] == 0)
        {
            self.auditHistoryDetailObj.auditVehicleMileage = @"Mileage?";
        }
        else
        {
            self.auditHistoryDetailObj.auditVehicleMileage = currentNodeContent;
        }
    }
    if([elementName isEqualToString:@"Colour"])
    {
        if([currentNodeContent length] == 0)
        {
             self.auditHistoryDetailObj.auditVehicleColor = @"Color?";
        }
        else
        {
            self.auditHistoryDetailObj.auditVehicleColor = currentNodeContent;
        }
    }
    if([elementName isEqualToString:@"VehicleType"])
    {
        if([currentNodeContent length] == 0)
        {
            self.auditHistoryDetailObj.auditVehicleType = @"Vehicle type?";
        }
        else
        {
            self.auditHistoryDetailObj.auditVehicleType = currentNodeContent;
        }
    ...
    if([elementName isEqualToString:@"Audit"])
    {
        ...
        [arrayOFDateWiseAudit addObject:self.auditHistoryDetailObj];
    }
    if([elementName isEqualToString:@"Count"])
    {
        totaCount = currentNodeContent.intValue;
    }
...
{
    NSLog(@"MainArray count = %lu",(unsigned long)arrayOFDateWiseAudit.count);
    
    
    if(arrayOFDateWiseAudit.count == 0)
    {
...
{
    SMStockAuditDetailObject *photoImagePath = (SMStockAuditDetailObject*)[arrayOFDateWiseAudit objectAtIndex:indexRow];
