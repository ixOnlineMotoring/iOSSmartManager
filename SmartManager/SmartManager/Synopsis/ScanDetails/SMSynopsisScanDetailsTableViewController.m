//
//  SMSynopsisScanDetailsTableViewController.m
//  Smart Manager
//
...
#import "SMSynopsisScanDetailsTableViewController.h"
...
@interface SMSynopsisScanDetailsTableViewController ()
{
    ...
    IBOutlet UILabel *lblKilometers;
    
    IBOutlet UILabel *lblExtrasCost;
    
    IBOutlet UILabel *lblCondition;
    
    IBOutlet UIView *viewHoldingBottomFields;
...
@implementation SMSynopsisScanDetailsTableViewController
...
- (void)viewDidLoad {
    ...
    txtVariant.delegate = self;
    txtKilometers.delegate = self;
    txtExtraCost.delegate = self;
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    switch (indexPath.row) {
        ...
        case 9:{
            ...
            if([self.VINLookupObject.strKiloMeters isKindOfClass:[NSNull class]] || [self.VINLookupObject.strKiloMeters length] == 0)
                {
                    return 0;//Change by doctor Ankit
                }
                else
                {
                    return 42;
...
- (void)textFieldDidBeginEditing:(UITextField *)textField
{
    ...
    if(textField == txtCondition)
    {
        ...
        [[UIApplication sharedApplication].windows[0] addSubview:popUpView];
    ...
    else if(textField == txtYear)
    {
        ...
        [popUpView getTheDropDownData:[SMAttributeStringFormatObject getDropDownArray:yearArray] withVariant:NO andIsPagination:NO ifSort:NO andIsFirstTimeSort:YES]; // array to be passed for custom popup dropdown
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string{
    ...
    {
        return [SMAttributeStringFormatObject valdateTextFeild:txtKilometers shouldChangeCharactersInRange:range replacementString:string withValidationType:ACCEPTABLE_CHARACTERS_Number andLimit:6];
        
    }
    if(textField == txtExtraCost)
    {
         return [SMAttributeStringFormatObject valdateTextFeild:txtExtraCost shouldChangeCharactersInRange:range replacementString:string withValidationType:ACCEPTABLE_CHARACTERS_Number andLimit:0];
    }
...
-(BOOL)validateBlankTextForButton:(int)tag{
    ...
    else if(!txtKilometers.isHidden && [txtKilometers.text isEqualToString:@""] && self.isFromScanPage){
    ...
    if ([elementName isEqualToString:@"ListVariantsJSONResponse"])
    {
        if (variantArray.count!=0)
        {
           ...
           [[UIApplication sharedApplication].windows[0] addSubview:popUpView];
            ...
            SMAlert(kTitle,@"No Variant(s) found");
        }
    }
                 ...
                 switch (selectedIndex)
                 {
                     case 0:{
                         ...
                         for (UIViewController *aViewController in allViewControllers) {
                             if ([aViewController isKindOfClass:[SMSynopsisVINScanViewController class]]) {
